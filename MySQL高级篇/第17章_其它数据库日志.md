...
通过错误日志可以查看系统的运行状态，便于即时发现故障、修复故障。如果MySQL服务`出现异常`
错误日志是发现问题、解决故障的`首选`。
...
```mysql
2022-01-84T08∶44:58.307609Z 0[System] [MY-010116] [Server] /usr/sbin/mysqld (mysqld 8.0.26)starting as process 1347
2022-01-64T08∶44:58.324982Z 1 [System] [NY-013576][InnoDB] InnoDB initialization has started.
2022-01-04T08∶44∶58.615451Z 1[System] [NY-813577][InnoDB] InnoDB initialization has ended.
2022-01-04T08∶44∶58.858832Z 0 [Warning] [WY-013746] [Server ] A deprecated TLS version TLSv1 isenabled for channel mysql_main
```


可以看到，错误日志文件中记录了服务器启动的时间，以及存储引擎lInnoDB启动和停止的时间等。我们在做初始化时候生成的数据库初始密码也是记录在error.log中。
...
![image-20220728170954478](https://jhblog-image.oss-cn-chengdu.aliyuncs.com/blogImg/image-20220728170954478.png)
...
- MySQL 5.5.7以前的版本，flush-logs将错误日志文件重命名为`filename.err_old`，并创建新的日志文件。
- 从MySQL 5.5.7开始，flush-logs只是重新打开日志文件，并不做日志`备份和创建`的操作。
...
1 行于数据集 (0.02 秒)
```
所有对数据库的修改都会记录在binglog中。但binlog是二进制文件，无法直接查看，借助`mysqlbinlog命令工具`了。指令如下:在查看执行，先执行一条sQL语句，如下
...
```mysql
mysqlbinlog  "/var/lib/mysql/lqhdb-binlog.000002"

#二进制数据
#220105 9:16:37 server id 1 end_log_pos 324 CRC32 0x6b31978b Querythread_id=10exec_time=0error_code=6
...
SET
0session.character_sep_client=33, @session.collation_connection=33,@@session.collation_server=255/*!*/;
...
#at 391
#220105 9:16:37 server id 1 end_log_pos 470 CRC32 8xc9920491 Update_rows: table id
85 flags :
STMT_END_F
BINLOG
dfHUYRNBAAAAQwAAAIcBAAAAFUAAAAAAAEACWFBZ3VpZ3UxNAAHc3R1ZGVudAADAw8PBDWAHgAGAQEAAgEhkJj4dA==
...
```mysql
mysqlbinlog -v "/var/lib/mysql/binlog/atguigu-bin.000002"  #添加"-v"
...
使用`RESET MASTER`语句，清空所有的binlog日志。MySQL会重新创建二进制文件，新的日志文件扩
展名将重新从000001开始编号。`慎用`!
...
为了安全起见，可以设置为 `1` ，表示每次提交事务都会执行fsync，就如同 **redo log 刷盘流程** 一样。最后还有一种折中方式，可以设置为`N`(N>1)，表示每次提交事务都write，但累积`N个事务后才fsync`。
